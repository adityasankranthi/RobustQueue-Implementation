...
public abstract class TestCollection<E> extends LockedTestCase {
	...
	protected boolean 
	    ...
	    preserveOrder = true,
	    failFast = true,
	    robust = false, // robust and failFast cannot both be true
	    hasRemove = true;
	...
	public void test28() {
		...
		if (failFast) {
			...
			E obj5 = it2.next();
			if (preserveOrder) {
				assertEquals(e[4],obj4);
				assertEquals(e[6],obj5);
			}
		}
	}
	...
	// Now some tests of add with robust iterators
	...
	public void test74()
	{
		...
		if (failFast) {
			...
			assertTrue(it2.hasNext());
			assertException(IllegalStateException.class, () -> it2.remove());
			E object = it2.next();
			if (preserveOrder) assertEquals(e[8],object);
		}
	...
	public void test75() {
		...
		if (failFast) {
			...
			assertFalse(it.hasNext());
			assertException(IllegalStateException.class, () -> it2.remove());
			assertException(NoSuchElementException.class, () -> it2.next());
		}
	...
	public void test79() {
		...
		if (failFast) {
			...
			// iterator was at the last element which was removed, so the new element isn't seen
			assertTrue(it.hasNext());
		}
		assertEquals(1,c.size());
	}
